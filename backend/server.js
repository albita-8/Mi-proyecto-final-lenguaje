...
app.get('/api/personajes/:id', (req, res) => {
  ...
  db.query(sql, [id], (err, results) => {
    ...
    // Obtener las películas en las que aparece el personaje
    const sqlPeliculas = `
      ...
      WHERE pp.CodPer = ?
    `;
    db.query(sqlPeliculas, [id], (err2, peliculas) => {
      if (err2) {
...
// PUT /api/personajes/:id — Actualizar un personaje existente
...
app.delete('/api/personajes/:id', (req, res) => {
  const { id } = req.params;
  // Primero eliminar sus relaciones en peli_pers para no violar la FK
...
// GET /api/peliculas/:id — Detalle de una película con sus personajes
app.get('/api/peliculas/:id', (req, res) => {
  const { id } = req.params;
  db.query('SELECT * FROM pelicula WHERE CodPel = ?', [id], (err, results) => {
    if (err) {
    ...
    const sqlPersonajes = `
      ...
      WHERE pp.CodPel = ?
    `;
    db.query(sqlPersonajes, [id], (err2, personajes) => {
      if (err2) {
        return res.status(500).json({ error: 'Error al obtener los personajes de la película.', detalle: err2.message });
      }
